...
def load_vocab(vocab_file: str):
    ...
    with open(vocab_file, "r", encoding="utf-8") as f:
        for line in f:
            line = line.strip()
            # Skip empty lines to avoid unpacking errors
            if not line:
                continue
            
            try:
                # Use rsplit in case the token itself contains a space
                token, index = line.rsplit(maxsplit=1)
                vocab[token] = int(index)
            except ValueError:
                print(f"Warning: Skipping malformed line in vocab file: '{line}'")
