# -*- coding: utf-8 -*-
import re
from convert_num2text.num_dict import config_dict
from convert_num2text.util import *

NUM = config_dict["NUM"]
NUM_TY = config_dict["NUM_TY"]
NUM_UNIT = config_dict["NUM_UNIT"]
NUM_OPERATOR = config_dict["NUM_OPERATOR"]
COMMON = config_dict["COMMON"]
MONTH = config_dict["MONTH"]
TEL_PREFIX = config_dict["TEL_PREFIX"]
...
#     text = re.sub(r"[$៛€¥￥₹£฿₫₭]", lambda m: CURRENCY.get(m.group()), text)
#     return text

def textNorm(text):
    # # Replace English digits with Khmer digits
    text = re.sub(r"[0-9]+", num_en2km, text)

    # Safely handle currency symbols BEFORE converting Khmer numbers
    text = re.sub(r"([$៛€¥￥₹£฿₫₭])", lambda m: CURRENCY.get(m.group(), m.group()), text)

    # Convert Khmer numerals to words
    text = re.sub(r"[+-]?([០-៩]*[,])?[០-៩]+", num2word, text)

    # Add Khmer punctuation if not present
    punctuation = r"!?,.;។"
    if not text or text[-1] not in punctuation:
        text += ';'
